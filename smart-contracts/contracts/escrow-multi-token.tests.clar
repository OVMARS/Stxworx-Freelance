;; ========================================================================
;; Rendezvous Fuzz & Invariant Tests for escrow-multi-token-v4
;; Run with: npx rv . escrow-multi-token-v4 test --runs=500
;;           npx rv . escrow-multi-token-v4 invariant --runs=500
;; ========================================================================
...
(define-public (test-create-project-always-increments-counter (amount1 uint) (amount2 uint))
  (let (
    ...
    (count-before (contract-call? .escrow-multi-token-v4 get-project-count))
  )
    ;; Create a project as tx-sender (assumes tx-sender != 'ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG)
    (match (contract-call? .escrow-multi-token-v4 create-project-stx
              'ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG a1 a2 u0 u0)
      ok-val
        (begin
          (asserts! (is-eq (contract-call? .escrow-multi-token-v4 get-project-count) (+ count-before u1))
            (err u9001))
          (ok true))
...
(define-public (test-fee-always-within-bounds (amount uint))
  (let (
    (fee-rate (contract-call? .escrow-multi-token-v4 get-fee-rate))
    ...
    (match (map-get? milestones {project-id: pid, milestone-num: mnum})
      milestone-data
        (if (get released milestone-data)
          ...
          (match (contract-call? .escrow-multi-token-v4 release-milestone-stx pid mnum)
...
(define-public (test-non-owner-admin-always-fails (fee-rate uint))
  (let (
    (owner (contract-call? .escrow-multi-token-v4 get-contract-owner))
  )
    (if (is-eq tx-sender owner)
      (ok true)  ;; Skip if tx-sender is owner
      (begin
        (match (contract-call? .escrow-multi-token-v4 set-fee-rate fee-rate)
...
(define-public (invariant-stx-balance-covers-obligations)
  (let (
    (balance (contract-call? .escrow-multi-token-v4 get-contract-balance-stx))
    (count (contract-call? .escrow-multi-token-v4 get-project-count))
...
(define-private (calc-stx-obligation (pid uint) (acc uint))
  (match (contract-call? .escrow-multi-token-v4 get-project pid)
    project
      (if (and (is-eq (get token-type project) u0) (not (get refunded project)))
        (match (contract-call? .escrow-multi-token-v4 get-refundable pid)
          ok-val (+ acc ok-val)
          err-val acc)
        acc)
    acc))
...
(define-public (invariant-committed-sbtc-consistency)
  (let (
    (committed (contract-call? .escrow-multi-token-v4 get-committed-sbtc))
