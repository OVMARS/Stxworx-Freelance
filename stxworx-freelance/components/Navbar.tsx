...
const Navbar: React.FC<NavbarProps> = ({
  ...
  }, [wallet.isConnected, fetchUnreadCount]);
  ...
  const NotificationDropdown = () => (
    <div className="absolute right-0 sm:right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 md:w-96 max-w-[24rem] bg-[#0b0f19] border border-slate-800 rounded-xl shadow-2xl shadow-black/40 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
  ...
  const NavLink = ({ view, label }: { view: ViewType; label: string }) => (
    ...
    >
      ...
      <span
        className={`absolute bottom-0 left-0 w-full h-0.5 bg-orange-500 transform transition-transform duration-300 ${
          currentView === view ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-50'
        }`}
      ></span>
      ...
      <div className="relative">
        <Icon 
          className={`w-[22px] h-[22px] transition-all duration-200 ${
            active ? 'text-orange-500 drop-shadow-[0_0_8px_rgba(249,115,22,0.6)]' : 'group-active:text-slate-300'
          }`} 
    ...
    <>
      <nav className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-14 md:h-20 lg:h-24 items-center gap-4">

            {/* ── NEW LOGO ── */}
            <div
              className="flex items-center cursor-pointer group shrink-0 transition-transform active:scale-95 hover:scale-[1.02]"
              onClick={() => onNavigate('home')}
            >
              <img
                src={STXWORXLogo}
                alt="STXWORX Logo"
                className="h-9 sm:h-10 md:h-12 lg:h-14 w-auto object-contain drop-shadow-[0_0_10px_rgba(249,115,22,0.5)] group-hover:drop-shadow-[0_0_16px_rgba(249,115,22,0.8)] transition-all duration-300"
              />
            </div>
